...
static void setup_thread(LIBEVENT_THREAD *me) {
    ...
    event_set(&me->notify_event, me->notify_receive_fd,
              EV_READ | EV_PERSIST, thread_libevent_process, me);//thread_libevent_process 事件处理
...
void thread_init(int nthreads, struct event_base *main_base) {
    ...
    pthread_cond_init(&init_cond, NULL);//初始化条件变量

    pthread_mutex_init(&cqi_freelist_lock, NULL);
    cqi_freelist = NULL;//连接队列

	//初始化每个线程 对应的消息和连接队列
    threads = malloc(sizeof(LIBEVENT_THREAD) * nthreads);//LIBEVENT_THREAD结构体
    if (! threads) {
    ...
    threads[0].thread_id = pthread_self();//这个线程自身是主线程啊????????

    for (i = 0; i < nthreads; i++) {
        int fds[2];
        if (pipe(fds)) {//生成读写文件描述符
