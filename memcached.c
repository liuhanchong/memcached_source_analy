...
int enable_large_pages(void) {
    int ret = -1;
    size_t sizes[32];
    int avail = getpagesizes(sizes, 32);//获取系统支持的页尺寸
    if (avail != -1) {
        size_t max = sizes[0];
...
int main (int argc, char **argv) {
    ...
    while ((c = getopt(argc, argv, "a:bp:s:U:m:Mc:khirvdl:u:P:f:s:n:t:D:L")) != -1) {
        ...
        case 'a':
            ...
            settings.access= strtol(optarg,NULL,8);/*访问 unix socket 和 -s选项并用*/
            break;

        case 'U':
            settings.udpport = atoi(optarg);/*监听的udp端口号*/
            break;
        case 'b':
            settings.managed = true;/*运行一个管理的实例*/
            break;
        case 'p':
            settings.port = atoi(optarg);/*tcp 端口号*/
            break;
        case 's':
            settings.socketpath = optarg;/*unix socket 监听的路径,和 -a选项并用*/
            break;
        case 'm':
            settings.maxbytes = ((size_t)atoi(optarg)) * 1024 * 1024;/*对象存储使用的最大内存，默认64m*/
            break;
        case 'M':
			/*不允许自动进行内存的清理（使用最近未经常使用算法），
			//但此时必须等有适合的内存出现才能加入新的缓存项目，鼻不建议禁用*/
            settings.evict_to_free = 0;
            break;
        case 'c':
            settings.maxconns = atoi(optarg);/*最大同时连接的数量，默认1024*/
            break;
        case 'h':
            usage();/*显示使用帮助*/
            exit(EXIT_SUCCESS);
        case 'i':
            usage_license();/*打印相关程序的版本信息*/
            exit(EXIT_SUCCESS);
        case 'k':
            lock_memory = true;/*锁住内存*/
            break;
        case 'v':
            settings.verbose++;/*在事件循环期间，打印错误和警告信息*/
            break;
        case 'l':
            settings.inter= strdup(optarg);/*监听的IP地址*/
            break;
        case 'd':
            daemonize = true;/*作为守护程序运行*/
            break;
        case 'r':
            maxcore = 1;/*提升核心文件数量至最大数*/
            break;
        case 'u':
            username = optarg;/*用户身份识别*/
            break;
        case 'P':
            pid_file = optarg;/*打印pidfile到optarg*/
            break;
        case 'f':
            settings.factor = atof(optarg);/*内存块的增长幅度，必须大于1*/
            ...
            break;
        case 'n':
            settings.chunk_size = atoi(optarg);/*为每个chunk块分配的最小内存，默认48*/
            ...
            break;
        case 't':
            settings.num_threads = atoi(optarg);/*处理进入进程的请求的线程数，默认4*/
        ...
        case 'D':
            ...
            settings.prefix_delimiter = optarg[0];/*键和ID分隔符号，默认:*/
            settings.detail_enabled = 1;/*描述激活*/
            break;
#if defined(HAVE_GETPAGESIZES) && defined(HAVE_MEMCNTL)
        case 'L' :
            if (enable_large_pages() == 0) {/*使用最大内存页*/
                preallocate = true;/*标记*/
            }
            break;
#endif
    ...
    //对于root权限不可以
    if (getuid() == 0 || geteuid() == 0) {//root用户
        if (username == 0 || *username == '\0') {//不能以root启动
    ...
    main_base = event_init();//返回主线程libevent当前全局实例
   ...
   //初始化memcached所需要的模块
    ...
    conn_init();//memca~.c
    suffix_init();//memca~.c
    slabs_init(settings.maxbytes, settings.factor, preallocate);//slabs.c
     ...
     */
    sa.sa_handler = SIG_IGN;/*指向空的操作函数*/
    sa.sa_flags = 0;
	//初始化信号集并设置信号集
    if (sigemptyset(&sa.sa_mask) == -1 ||
        sigaction(SIGPIPE, &sa, 0) == -1) {//处理断开的socket操作的信号处理函数
    ...
    if (daemonize)//后台运行
        save_pid(getpid(), pid_file);//将进程的ID写入文件中，在后台运行时可以快速操作
